...
import ua.agentlab.lab3.employment.Address;
import ua.agentlab.lab3.employment.Company;
import ua.agentlab.lab3.employment.EmploymentOntology;
import ua.agentlab.lab3.employment.Engage;
import ua.agentlab.lab3.employment.EngagementError;
import ua.agentlab.lab3.employment.MoscowIsForbidden;
import ua.agentlab.lab3.employment.Person;
import ua.agentlab.lab3.employment.PersonTooOld;
import ua.agentlab.lab3.employment.WorksFor;
	...
	*/
	...
	public ACLMessage prepareResponse(ACLMessage request) {
			...
			try{
				...
				// Check person's age. If < 35 --> AGREE, else REFUSE and exit
				boolean isFromMoscow = "moscow".equalsIgnoreCase(p.getAddress().getCity());
				if (p.getAge().intValue() < 35 && !isFromMoscow){
					...
					ContentElementList l = new ContentElementList();
					l.add(a);
					l.add(isFromMoscow ? new MoscowIsForbidden() : new  PersonTooOld());
					getContentManager().fillContent(temp, l);
	...
	public EngagerAgent(){
		...
		representedCompany.setName("ELEKS");
		...
		manager.setName("Oleksiy Skrypnyk");
		manager.setAge(new Long(52));
		manager.setAddress(a);
		representedCompany.setManager(manager);
