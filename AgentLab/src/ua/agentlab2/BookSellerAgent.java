...
public class BookSellerAgent extends Agent {
	 ...
	 * @param cardNumber 
	 */
	public void updateCatalogue(final String title, final int price, final Integer cardNumber, final Integer distPercent) {
		addBehaviour(new OneShotBehaviour() {

			private static final long serialVersionUID = -4297468131623056280L;
	...
	private class OfferRequestsServer extends CyclicBehaviour {
		...
		private static final long serialVersionUID = -4096147477401364069L;
		...
		public void action() {
			...
			if (msg != null) {
				...
				if(message.contains("|")){
					...
					cardNumber = Integer.valueOf(message.substring(message.indexOf("|") + 1, message.length()));
				}
				Integer price = (Integer) catalogue.get(title);
				
				Integer discount = discounts.get(cardNumber);
				if(cardNumber != null && discount != null) {
					price = price - price * discount/100;
					System.out.println("Client = " + msg.getSender() + " has discount = " + discount + " for card = " + cardNumber );
				}
				if (price != null) {
	...
	private class PurchaseOrdersServer extends CyclicBehaviour {
		...
		private static final long serialVersionUID = -5070547226750612159L;
		...
		public void action() {
			...
			if (msg != null) {
				...
				ACLMessage reply = msg.createReply();
				Integer price = (Integer) catalogue.remove(bookTitle);
				if (price != null) {
					reply.setPerformative(ACLMessage.INFORM);
					System.out.println(bookTitle + " sold to agent " + msg.getSender().getName());
